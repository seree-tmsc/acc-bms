...
<html lang="en">
    ...
    <body>
        <?php
            ...
            // Function
            // ---------------------
            function f_Status_Of_Table_MAS_CUSTOMER_PAYMENT_SCHEDULE($nPaymentYear)
            {
                require('include/db_Conn.php');

                $strSql = "SELECT * ";
                $strSql .= "FROM " . "MAS_CUSTOMER_PAYMENT_SCHEDULE_" . $nPaymentYear . "  ";
                //echo $strSql . "<br>";
            ...
            function f_Create_Array_Of_CUSTOMER_PAYMENT_SCHEDULE($ds, $nPaymentYear)
            {
                        ...
                        switch($ds['day_frequency'])
                        {
                            ...
                            case 'ALL':
                                ...
                                for($nI=1; $nI<=12; $nI++)
                                {
                                    ...
                                    if($nI<10)
                                    {
                                        $cCurMonth = $nPaymentYear . '/0' . $nI . '/01';
                                    }
                                    else
                                    {
                                        $cCurMonth = $nPaymentYear . '/' . $nI . '/01';
                                    }
                                    ...
                                    for($nJ=1; $nJ<=$nNumberOfDayOfThisMonth; $nJ++)
                                    {
                                        ...
                                        if($nI<10)
                                        {
                                            if($nJ<10)
                                            {
                                                $cCurDate = $nPaymentYear . '/0' . $nI . '/0' . $nJ;
                                            }
                                            else
                                            {
                                                $cCurDate = $nPaymentYear . '/0' . $nI . '/' . $nJ;
                                            }
                                        }
                                        else
                                        {
                                            if($nJ<10)
                                            {
                                                $cCurDate = $nPaymentYear . '/' . $nI . '/0' . $nJ;
                                            }
                                            else
                                            {
                                                $cCurDate = $nPaymentYear . '/' . $nI . '/' . $nJ;
                                            }
                                        }
                        ...
                        switch($cTmpString)
                        {
                            ...
                            case 'END':
                                ...
                                for($nI=1; $nI<=12; $nI++)
                                {
                                    ...
                                    for($nJ=1; $nJ<=5; $nJ++)
                                    {
                                        $cTmpLastDayOfWeekOfMonth = "last " . $aDayName[$nJ-1] . " of " . $aMonthName[$nI-1] . ' ' .  $nPaymentYear;
            ...
            function f_Start_Process_Insert_Data_Into_MAS_CUSTOMER_PAYMENT_SCHEDULE($ds, $nPaymentYear, $aCustomerPaymentSchedule)
            {
                    ...
                    for($nI=1; $nI<=12; $nI++)
                    {
                        ...
                        for($nJ=1; $nJ<=$nNumberOfDayOfThisMonth; $nJ++)
                        {
                            ...
                            if(f_Not_TMSC_Holiday($cCurDate))
                            {
                                ...
                                if(in_array(date('d', strtotime($cCurDate)), $aCustomerPaymentSchedule[$nI-1]))
                                {
                                    echo $cCurDate . ", ";

                                    require('include/db_Conn.php');

                                    $strSql1 = "INSERT INTO MAS_CUSTOMER_PAYMENT_SCHEDULE_" .  $nPaymentYear . " VALUES(";
                                    $strSql1 .= "'" . $ds['cust_code'] . "', ";
                                    $strSql1 .= "" . $ds['condition_no'] . ", ";
                                    $strSql1 .= "'" .  $nPaymentYear . "', ";
            ...
            function f_Not_TMSC_Holiday($dCheckDate)
            {
                ...
                $strSql = "SELECT * ";
                $strSql .= "FROM " . "MAS_TMSC_HOLIDAY_" . $_POST['nPaymentYear'];
            ...
            function f_Insert_Record_Into_MAS_CUSTOMER_PAYMENT_SCHEDULE($nYear, $nMonth, $nDay, $cCust_Code, $nCondition_No)
            {
                require('include/db_Conn.php');
            ...
            // Main program
            // ---------------------
            echo "Create Table -- MAS_CUSTOMER_PAYMENT_SCHEDULE_" . $_POST['nPaymentYear'] . " --- <br>";
            
            // check ว่า table MAS_CUSTOMER_PAYMENT_SCHEDULE ว่างหรือเปล่า
            if (f_Status_Of_Table_MAS_CUSTOMER_PAYMENT_SCHEDULE($_POST['nPaymentYear']) == 'EMPTY')
            {
                ...
                if ($nRecCount > 0)
                {
                    ...
                    while($ds = $statement->fetch(PDO::FETCH_NAMED))
                    {
                        ...
                        $aCustomerBillSchedule = f_Create_Array_Of_CUSTOMER_PAYMENT_SCHEDULE($ds, $_POST['nPaymentYear']);
                        //print_r($aCustomerBillSchedule) . "<br>";

                        // insert data ลง table MAS_CUSTOMER_BILL_SCHEDULE
                        f_Start_Process_Insert_Data_Into_MAS_CUSTOMER_PAYMENT_SCHEDULE($ds, $_POST['nPaymentYear'], $aCustomerBillSchedule);
                    }
                    //ob_end_flush();
                    echo "<h3>Complete</h3>" . "<br>";
                }
            }
            // ถ้า table MAS_CUSTOMER_PAYMENT_SCHEDULE ไม่ว่าง แจ้ง user ให้ clear table ว่างแล้ว ดำเนินการอีกครั้ง
            else
            {
                echo "Status = FULL<br>";
                echo "Please contact administrator to delete data in Table -- MAS_CUSTOMER_PAYMENT_SCHEDULE_" . $_POST['nPaymentYear'] . " --- <br>";
